...
export function createOffer(verifier: Verifier, createOfferPath: string, offerPath: string, responsePath:string, presentationPath:string, checkPath:string) {
    ...
    verifier.router!.post(createOfferPath,
        ...
        try {
            ...
            const requestByReferenceURI = getBaseUrl() + replaceParamsInUrl(offerPath, {presentationid: presentationId, state:session.uuid});
            const checkUri = getBaseUrl() + replaceParamsInUrl(checkPath, { presentationid: presentationId, state:session.uuid });
